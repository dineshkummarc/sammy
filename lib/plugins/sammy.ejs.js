(function($) {
  ...
  var srender = function(app, name, template, data) {
    var fn, parsed;
    ...
    if (app.cache_partials && app.cache('ejs:' + name)) {
      fn = new Function("obj", app.cache('ejs:' + name));
    } else {
      ...
      parsed = srender_parser(template);
      if (app.cache_partials)
        app.cache('ejs:' + name, parsed);
      fn = new Function("obj", parsed);
    }
  ...
  Sammy.Ejs = function(app) {
    var base_partial = app.context_prototype.prototype.partial;
    var ejs_requests = {};
    ...
    app.helpers({
      partial: function(path, data, callback, ajax_options) {
        ...
        return base_partial.apply(this, arguments);
      },
      ...
      ejs_partial: function(path, data, callback, ajax_options) {
        ...
        rendered = srender(this.app, path, undefined, data);
        if (rendered) {
          ...
          rendered = '<span class="promise" id="' + request_data.promise_id + '"></span>';

          ajax_options.success = function(template) {
            while (ejs_requests[path].length > 0) {
              var request = ejs_requests[path].shift();
              rendered = srender(context.app, path, template, request.data);
              if ($.isFunction(request.callback)) {
                request.callback(rendered);
              } else {
                context.ejs_render_promised(request.promise_id, rendered);
              }
            }
            delete ejs_requests[path];
          };

          if (!ejs_requests[path]) {
            ejs_requests[path] = [request_data];
            $.ajax($.extend({ url: path, dataType: 'text', type: 'GET' }, ajax_options));
          } else {
            ejs_requests[path].push(request_data);
          }
        }

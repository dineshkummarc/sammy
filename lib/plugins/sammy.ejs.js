(function($) {
  ...
  //
  $.srender_parser = function(template) {
  ...
  $.srender = function(name, template, data) {
    ...
    if ($.srender.cache[name]) {
      ...
      fn = $.srender.cache[name] = new Function("obj", $.srender_parser(template));
    }
...
Sammy.EventContext.prototype = $.extend(Sammy.EventContext.prototype, {
  ...
  ejs: function(path, data, callback, ajax_options) {
    ...
    if (typeof callback == 'undefined') {
      if ($.isFunction(data)) {
        ...
        callback = this.ejs_render_callback;
      }
    }
  ...
  // This callback will be used to apply the result of <tt>ejs</tt> if no
  // callback is passed.
  ejs_render_callback: function(data) {

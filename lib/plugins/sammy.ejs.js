  ...
  Sammy.Ejs = function(app) {
    ...
    app.helpers({
      ...
      ejs_partial: function(path, data, callback, ajax_options) {
        var first_request, rendered, promise_id, context = this;
        ...
        if (rendered) {
          ...
          first_request = !ejs_requests[path];

          ajax_options.success = function(template) {
            rendered = srender(context.app, path, template, data);
            if ($.isFunction(callback)) callback(rendered);
            if (first_request) {
              while (ejs_requests[path].length > 0) {
                ejs_requests[path].shift()(template);
              }
          ...
          if (first_request) {
            ...
            ejs_requests[path].push(ajax_options.success);
          }
        }
